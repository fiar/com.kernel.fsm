...
namespace Kernel.FSM
{
    ...
    public interface IStateBuilder<T, TParent> where T : AbstractState, new()
    {
		...
		/// <summary>
		/// Set an action to be called when we awake the state.
		/// </summary>
		IStateBuilder<T, TParent> Awake(Action<T> onAwake);
		...
		/// <summary>
		/// Set an action to be called when we destroy the state.
		/// </summary>
		IStateBuilder<T, TParent> Destroy(Action<T> onDestroy);
        ...
        where T : AbstractState, new()
    {
		...
		/// Set an action to be called when we awake the state.
		/// </summary>
		public IStateBuilder<T, TParent> Awake(Action<T> onAwake)
		{
			state.SetAwakeAction(() => onAwake(state));
		...
		/// Set an action to be called when we destroy the state.
		/// </summary>
		public IStateBuilder<T, TParent> Destroy(Action<T> onDestroy)
		{
			state.SetDestroyAction(() => onDestroy(state));
